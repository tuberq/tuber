...
impl ServerState {
    ...
    pub(super) fn restore_jobs(
        ...
        let buried_rank: HashMap<u64, usize> = buried_order
            ...
            .map(|(rank, &id)| (id, rank))
            .collect();
        let mut buried_pending: Vec<(usize, u64, std::sync::Arc<str>)> = Vec::new();
